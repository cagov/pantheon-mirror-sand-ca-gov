...
function mcm_get_count_for_media_taxonomy( $media_taxonomy = '' ) {
	global $wpdb;
	...
	$taxonomyTerms = $wpdb->get_results( $query );
	mcm_debugMP('pr',__FUNCTION__ . ' media_taxonomy ' . $media_taxonomy . ' found ' . count($taxonomyTerms) . ' terms with query = ' . $query, $taxonomyTerms);
...
//	$query .= " GROUP BY $wpdb->posts.ID";
	$taxonomyPosts = $wpdb->get_results( $query );
	mcm_debugMP('pr',__FUNCTION__ . ' media_taxonomy ' . $media_taxonomy . ' found ' . count($taxonomyPosts) . ' posts with query = ' . $query, $taxonomyPosts);
...
function mcm_get_media_taxonomies() {
	...
	foreach ($mediaTaxonomiesFound as $taxonomyObject) {
		...
		$countMediaPosts = mcm_get_count_for_media_taxonomy( $taxonomySlug );
		mcm_debugMP('msg',__FUNCTION__  . ' taxonomySlug ' . $taxonomySlug . ' found countMediaPosts = ' . $countMediaPosts . ' !' );

		// Get the taxonomy information
		$mediaTaxonomy = get_taxonomy($taxonomySlug);
		mcm_debugMP('pr',__FUNCTION__  . ' taxonomySlug ' . $taxonomySlug . ', mediaTaxonomy found:', $mediaTaxonomy);
...
//	return '4';
	...
	$all_attachments = mcm_get_posts_for_media_taxonomy($media_taxonomy);
	$post_not_in = array();
	foreach ($all_attachments as $key => $val) {
		$post_not_in[] = $val->ID;
	}
	$attachments_args['post__not_in'] = $post_not_in;
	mcm_debugMP('pr', __FUNCTION__ . ' post_not_in = ', $post_not_in);
...
function mcm_get_attachment_ids( $mcm_atts = array() ) {
	...
	mcm_debugMP('pr',__FUNCTION__ . ' wp_mcm_default_media_category = ' , $media_categories);

	// Check to find the media that have no category assigned
	if ( $media_categories[0] == WP_MCM_OPTION_NONE ) {
		$attachment_ids_result = mcm_get_attachment_ids_no_category( $mcm_atts );
		mcm_debugMP('pr',__FUNCTION__ . ' attachment_ids_result for WP_MCM_OPTION_NONE = ' . $attachment_ids_result );
		return $attachment_ids_result;
	}
