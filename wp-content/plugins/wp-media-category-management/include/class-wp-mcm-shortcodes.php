...
class WP_MCM_Shortcodes {
    ...
    function get_wp_mcm_shortcodes() {
		...
		$wp_mcm_default_media_category_value = mcm_get_option( 'wp_mcm_default_media_category' );
        ...
        return array (
					array(
						...
						'parameters'  => '<ul>' .
											...
											'<br/>' .
											sprintf(__('The default value is as defined in MCM Settings for <strong>Default Media Category</strong>, currently defined as <code>%s</code>.', 'wp-media-category-management'), $wp_mcm_default_media_category_value) .
											'<br/>' .
											sprintf(__('When the default value is <code>%s</code>, only the media are shown that have no category assigned.', 'wp-media-category-management'), $wp_mcm_default_media_category_text) .
											'</li>' .
											'<li>' .
	...
	/**
	 * Handles the WP_MCM shortcode. The short code takes the following arguments:
